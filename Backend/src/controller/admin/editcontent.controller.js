...
// Create hero section
const add_hero_section = asyncHandler(async (req, res) => {
  ...
  return res.status(201).json(
    new ApiResponse(201, savedSection, "Hero section created successfully")
  );
});

// Get all hero sections
const get_hero_sections = asyncHandler(async (req, res) => {
  const sections = await HeroSection.find().sort({ createdAt: -1 });
  return res.status(200).json(
    new ApiResponse(200, sections, "Hero sections fetched successfully")
  );
});

// Update hero section
const update_hero_section = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const { surprise_text, sub_text, description } = req.body;
  ...
  return res.status(200).json(
    new ApiResponse(200, updatedSection, "Hero section updated successfully")
  );
});

// Delete hero section
const delete_hero_section = asyncHandler(async (req, res) => {
  const { id } = req.params;
